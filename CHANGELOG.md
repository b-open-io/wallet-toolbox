...
## @bopen-io/wallet-toolbox 2.1.21-parity-fix.1 (draft — not yet published)

Canonical + StorageIdb fixes from the 2026-04-22 overnight parity pass. All fixes apply
**forward behavior only** — existing dirty data in deployed wallets is recovered by
remote-primary re-sync, not by migration code. Knex remains canonical; IDB (and BunSqlite,
handled separately in `@1sat/wallet-node`) are aligned to Knex.

**Canon (all three providers)**

- **CRIT-1** `internalizeAction` now computes and persists `scriptOffset`/`scriptLength`
  in every output writer (`storeNewWalletPaymentForOutput`, `mergeWalletPaymentForOutput`,
  `storeNewBasketInsertionForOutput`, `mergeBasketInsertionForOutput`). Matches
  `processAction`. Enables the `validateOutputScript` rawTx-slice fallback to apply
  uniformly to internalized outputs.
- **CRIT-2** `updateTransactionStatus('failed')` now also marks the failing tx's own
  outputs `spendable=false, basketId=null` after restoring inputs. Closes the "ghost
  output in basket" case (Dan's BAP basket).
- **HIGH-13** `StorageReaderWriter.findOrInsert*` helpers now thread `trx` into every
  inner update/insert call. Covers `findOrInsertUser` (inner basket insert),
  `findOrInsertOutputBasket`, `findOrInsertTxLabel`, `findOrInsertTxLabelMap`,
  `findOrInsertOutputTag`, `findOrInsertOutputTagMap`, `findOrInsertSyncStateAuth` (new
  optional `trx?` param). Eliminates IDB deadlock risk when an un-delete branch fires
  inside `transaction()`.
- **LOW-20** `EntityProvenTxReq.mergeExisting` now only writes when `batch`/`history`/
  `notify` actually changed and returns `true` on real write. `updates` counter is no
  longer under-reported.
- **LOW-21** `EntityOutputBasket.equals` now includes `isDeleted` so sync-drift
  verification matches what `mergeExisting` propagates.
- **LOW-27** `StorageProvider.validateOutputScript` emits a `console.debug` when guard 1
  trips on an output with neither `lockingScript` nor `scriptOffset` — gives future
  debuggers a signal for the CRIT-1 failure mode.

**StorageIdb**

- **CRIT-1b** `findOutputs(noScript:true)` no longer wipes `lockingScript` that IDB had
  on the row. Parity with Knex/Bun (which simply don't SELECT the column).
- **HIGH-7** `listOutputs` tx-status allowlist now includes `'sending'`. Outputs of
  mid-broadcast transactions stay visible on IDB, matching Knex/Bun.
- **HIGH-8** `getTagsForOutputId` / `getLabelsForTransactionId` use `verifyOneOrNone`
  with silent skip + `console.debug` for orphan map rows (instead of throwing). Matches
  Knex/Bun JOIN-drop.
- **HIGH-11** `updateIdb` silent-skips missing ids in batch updates and returns the real
  affected count. Matches Knex/Bun's `WHERE id IN (...)` semantics.
- **MED-14** `allocateChangeInput` passes `noScript:true` to the candidate scan and
  explicitly calls `validateOutputScript` on the chosen output — matches Knex's
  SELECT-then-hydrate pattern and closes the latent long-script-template gap.
- **MED-16** `listOutputs` negative offset reverses the IDB cursor direction and
  reindexes the offset (tail pagination). `filterOutputs` gains
  `args.orderDescending` → cursor direction support.
- **MED-18** `findTransactions(noRawTx:true)` only strips `rawTx`. `inputBEEF` is
  preserved; callers like `getReqsAndBeefToShareWithWorld` that need BEEF without
  rawTx now work on IDB.
- **MED-19** `insertCertificate` strips the non-schema `logger` runtime field before
  writing.
- **LOW-28** `getRawTxOfKnownValidTransaction` uses the `'unfail'`-inclusive status set
  on the script-slice path, matching Knex at `StorageKnex.ts:131`.

**Skipped / downgraded after validation**

- **CRIT-3** (`purgeDataIdb` no-op stub) — deferred. Latent, not regressing anyone
  tonight. Needs a focused standalone PR with four-phase cascade tests.
- **CRIT-5** (BunSqlite boolean bind) — NON-ISSUE. Driver probe (report at
  `docs/plans/2026-04-22-crit5-bun-sqlite-boolean-probe.md`) showed `bun:sqlite`
  coerces JS `true`/`false` → `1`/`0` cleanly. No code change.
- **HIGH-6** (receiver-side unmapped-parent guards) — DEFERRED + CLOSED via audit. The
  delete-path cascade audit (`docs/plans/2026-04-22-high6-delete-cascade-audit.md`)
  shows TS canon has no path that produces orphan children in the sender's DB.
- **HIGH-9** (`partial:{x:null}` divergence) — NOT-AN-ISSUE. Grep audit
  (`docs/plans/2026-04-22-high9-partial-null-audit.md`) found zero callers.
- **LOW-22/24/25** — by-design or requires standalone architectural work (BunSqlite
  SAVEPOINT vs real transactions).
